// ═══════════════════════════════════════════════════════
      ...
      const FarolHelpers = {
        ...
        cartaoStatus(c, baseDate) {
          ...
          const fech = parseInt(c.diaFechamento, 10) || 1;
          const venc = parseInt(c.diaVencimento, 10) || 1;

          // Última data de fechamento confirmada (mesRef da última fatura fechada)
          const faturas = Array.isArray(c.faturasFechadas)
            ? c.faturasFechadas
            : [];
          const ultima = faturas.length ? faturas[faturas.length - 1] : null;

          // Verifica se já passou do dia de fechamento neste mês e ainda não foi confirmado para este ciclo
          let diasParaFechar = fech - today;
          if (diasParaFechar < 0)
            diasParaFechar += this.daysInMonth(
              now.getFullYear(),
              now.getMonth(),
            );

          const cicloAtualKey = `${now.getFullYear()}-${now.getMonth()}`;
          const jaFechouEsteMes = today >= fech;
          const aindaNaoConfirmou = !ultima || ultima.mesRef !== cicloAtualKey;
          const pendenteFechamento =
            jaFechouEsteMes && aindaNaoConfirmou && (c.faturaAtual || 0) > 0;

          // Calcula a data exata do próximo vencimento
          const proxVencimento = this.proximoVencimento(venc, now);
        ...
        // Retorna a data do próximo vencimento (Date)
        proximoVencimento(diaVenc, baseDate) {
          const now =
            baseDate instanceof Date ? new Date(baseDate) : new Date();
          const today = now.getDate();
          let ano = now.getFullYear();
          let mes = now.getMonth();
          // Se o dia de vencimento já passou esse mês, vai para o próximo mês
          if (today > diaVenc) {
            mes += 1;
            if (mes > 11) {
              mes = 0;
          ...
          const dia = Math.min(diaVenc, diasNoMes);
        ...
        syncRecurring(key) {
          ...
          // Caso 1 — id temporário trocado pelo id do banco: o gasto antigo
          // (templateId com o id temporário) fica órfão no ciclo, causando
          // duplicação a cada recarregamento.
          // Caso 2 — lixo legado de versões antigas do app, que escreviam
          // o gasto da parcela direto em ciclo.gastos SEM templateId. Esse
          // gasto nunca é reconhecido como "da parcela" e fica para sempre
          // ao lado do gasto correto (gerado via templateId), duplicando
          // a exibição. Detectamos esse caso por coincidência exata de
          // descrição + valor + data com um gasto de parcela válido — e só
          // removemos se o gasto legado não estiver marcado como pago, para
          // nunca apagar um registro de pagamento real do usuário.
          if (Array.isArray(ciclo.gastos)) {
            ...
            // para poder comparar contra eles na 2ª passada.
            const gastosParcelaValidos = ciclo.gastos.filter(
              (g) =>
                g.templateId &&
            ...
            ciclo.gastos = ciclo.gastos.filter((g) => {
              if (g.templateId && g.templateId.startsWith("parcela:")) {
                if (!idsValidos.has(g.templateId)) return false; // órfão: parcela não existe mais
                if (vistos.has(g.templateId)) return false; // duplicado: já tem um deste templateId
                vistos.add(g.templateId);
                return true;
              }
              // Gasto sem templateId de parcela: verifica se é lixo legado
              // que duplica um gasto de parcela válido (mesma descrição,
              // valor e data) e não está marcado como pago.
              if (!g.pago) {
              ...
              return true; // gasto manual legítimo, mantém
            });
          }
          ...
          fixos.forEach((t) => {
            ...
            if (Array.isArray(t.skipMonths) && t.skipMonths.includes(key))
              return;
            const exists = ciclo.gastos.some((g) => g.templateId === t.id);
            if (!exists) {
          ...
          parcelas.forEach((p) => {
            ...
            if (Array.isArray(p.skipMonths) && p.skipMonths.includes(key))
              return;
            // Parcelas pagas no cartão já estão incluídas no valor da fatura — não somam de novo no mês.
            if (p.origem === "cartao") return;
            const tId = `parcela:${p.id}`;
            const exists = ciclo.gastos.some((g) => g.templateId === tId);
            if (!exists) {
              const venc = parseInt(
          ...
          // Quem precisa persistir o ciclo (ex: marcar pago) chama Store.set
          // explicitamente na própria ação do usuário.
