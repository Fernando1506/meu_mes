/*
  ...
  09-SCREENS.JS
  ═══════════════════════════════════════════════════════
  Renderização das telas principais. Cada Screens.nomeDaTela retorna o HTML daquela página.
...
// ═══════════════════════════════════════════════════════
      // SCREENS
      // ═══════════════════════════════════════════════════════
      /* Screens: cada função abaixo renderiza uma tela inteira do sistema. */
      const Screens = {};

      // Dashboard: visão executiva do mês, KPIs, alertas e próximos passos.
      ...
      // Meu Mês: gestão operacional de entradas, gastos, contas e decisões do ciclo.
      ...
      <div class="page-wrap">
        ...
        ${(() => {
          ...
          // Para o fluxo por recebimento, o que define a janela é o VENCIMENTO.
          // Ex.: fatura que fecha dia 15, mas vence dia 20, deve cair na 2ª entrada.
          // Cartões antigos continuam funcionando porque o fallback usa diaVencimento.
          const cartoes = Store.get("cartoes") || [];
          const gastoJanelaDia = (g) => {
            if (g.virtual && g.cartaoId) {
              const c = cartoes.find((x) => x.id === g.cartaoId);
              if (c && FarolHelpers._monthlyCardDate) {
                const base = g.data ? FarolHelpers.parseLocalDate(g.data) : new Date();
                return FarolHelpers._monthlyCardDate(c, "vencimento", base, false).getDate();
              }
              if (c)
                return (
                  parseInt(c.diaVencimento, 10) ||
                  parseInt(c.vencimento, 10) ||
                  dayOf(g.data)
                );
            }
            return dayOf(g.data);
          ...
          // Divide por janela de vencimento:
          // - vencimento antes do dia 20 entra na 1ª entrada;
          // - vencimento a partir do dia 20 entra na 2ª entrada.
          const gastos1 = todosGastos.filter((g) => gastoJanelaDia(g) < CORTE);
          const gastos2 = todosGastos.filter(
            (g) => gastoJanelaDia(g) >= CORTE,
          );
          ...
          // Entradas: divide EXCLUSIVAMENTE pela data informada no lançamento.
          // Regra:
          // - dia 01 a 19 → 1ª entrada
          // - dia 20 em diante → 2ª entrada
          //
          // Importante: não existe mais tentativa de "adivinhar" vale, adiantamento
          // ou segunda entrada pelo nome/ordem. Se duas entradas foram lançadas no
          // dia 05/07, as duas ficam na 1ª entrada.
          const ent1 = todasEntradas.filter((f) => dayOf(f.data) < CORTE);
          const ent2 = todasEntradas.filter((f) => dayOf(f.data) >= CORTE);
          ...
          const renderGasto = (g) => {
            ...
            if (g.virtual && g.cartaoId) {
              ...
              if (c) {
                ...
                if (FarolHelpers._monthlyCardDate) {
                  const fechDt = FarolHelpers._monthlyCardDate(c, "fechamento", base, false);
                  const vencDt = FarolHelpers._monthlyCardDate(c, "vencimento", base, false);
                  diaLabel = `fecha ${fmtDateObj(fechDt)} · vence ${fmtDateObj(vencDt)}`;
                } else {
                  const fech =
      ...
      // Análise: leitura estratégica dos gastos, blocos e recomendações.
      ...
      // Evolução: comparação histórica e tendências mês a mês.
      ...
      // Objetivos: metas financeiras e acompanhamento de progresso.
      ...
      // Parcelas: controle de compras parceladas e compromissos recorrentes.
          ...
          <div class="card mb-16">
            ...
            <div class="flex justify-between items-center mb-12" style="gap:12px;flex-wrap:wrap;">
              <div>
                ...
                <div class="text-sm text-grey mt-4">Fecha ${c.dataFechamento ? FarolHelpers.formatDate(c.dataFechamento) : `dia ${c.diaFechamento}`} · Vence ${c.dataVencimento ? FarolHelpers.formatDate(c.dataVencimento) : `dia ${c.diaVencimento}`}</div>
              </div>
              <div class="flex gap-8">
      ...
      // Princípios: orientações e critérios de decisão financeira.
